...
// Value implements the driver.Valuer interface for converting Point to database format.
func (p Point) Value() (driver.Value, error) {
	return fmt.Sprintf("Point(%f %f)", p.lng, p.lat), nil
}

// Scan implements the sql.Scanner interface for converting database format to Point.
func (p *Point) Scan(src interface{}) error {
	...
	var lng, lat float64
	_, err := fmt.Sscanf(s, "POINT(%f %f)", &lng, &lat)
	if err != nil {
		return err
	}
